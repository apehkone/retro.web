...
namespace Retro.Web.Security
{
        ...
        where TUser : IdentityUser
    {
        ...
        private readonly string usersLink;
        private readonly string usersSelfLink;
        private bool disposed;


        public UserStore(IDbConfig dbConfig) {
            client = new DocumentClient(new Uri(dbConfig.EndpointUrl), dbConfig.AuthorizationKey);
            databaseName = dbConfig.DataBaseId;
            var collection = InitializeCollection(Database.SelfLink, dbConfig.DocumentCollectionId);
        ...
        private Database Database { get { return database ?? (database = ReadOrCreateDatabase()); } }

        private IQueryable<TUser> Users { get { return client.CreateDocumentQuery<TUser>(usersLink); } }
        ...
        public async Task<TUser> FindAsync(UserLoginInfo login) {
            ...
            return
                await
                    Task.Run(
                        () => {
                            IEnumerable<TUser> query = client.CreateDocumentQuery<TUser>(usersLink, string.Format("SELECT * FROM Users u WHERE udf.HasLogin(u.Logins, '{0}', '{1}') = true", login.ProviderKey, login.LoginProvider)).AsEnumerable();
        ...
        public async Task DeleteAsync(TUser user) {
            ...
            Document doc = client.CreateDocumentQuery(usersLink).FirstOrDefault(u => u.Id == user.Id);
        ...
        public async Task<TUser> FindByIdAsync(string userId) {
            ...
            return await Task.Run(() => {
                                      TUser user = Users.Where(u => u.Id == userId)
                                          .AsEnumerable()
                                          .FirstOrDefault();
        ...
        public async Task<TUser> FindByNameAsync(string userName) {
            ...
            return await Task.Run(() => {
                                      TUser user = Users.Where(u => u.UserName == userName)
                                          .AsEnumerable()
                                          .FirstOrDefault();
        ...
        public Task<bool> IsInRoleAsync(TUser user, string roleName) {
            ...
            bool isInRole = user.Roles.Any(x => x.Equals(roleName));
        ...
        private Database ReadOrCreateDatabase() {
            Database[] databases = client.CreateDatabaseQuery()
        ...
        private DocumentCollection InitializeCollection(string databaseLink, string collectionId) {
            DocumentCollection[] collections = client.CreateDocumentCollectionQuery(databaseLink)
        ...
        private async Task UpdateUserAsync(TUser user) {
            dynamic doc = await GetSpecificUserAsync(user.Id);
            ...
            if (doc != null) {
                ...
                doc.Retrospectives = user.Retrospectives;
